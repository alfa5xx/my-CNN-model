...
#---------------------------------------------------------
def get_vehicle_speed_factor(edge_id, default_max_speed=15.89): # default value for default_max_speed=13.89
    #
...
def train_multi_input_model(X_train_image, X_train_scalar, Y_train, 
                          ...
                          start_edge, destination_edge, optimal_path=None):
    ...
    # plt.figtext(0.95, 0.7, epoch_info, fontsize=10, ha='left')  # default values : 0.95, 0.7, epoch_info
    ...
    #     f"SUMO start and end edges: paths from {start_edge} to {destination_edge}"
    # )
    #     # To add the optimal path, you need to pass it to the function or access it from a global variable
    # # Add this line after you've completed finding the optimal path
    # plt.figtext(0.01, 0.3, f"Optimal path: {' → '.join(optimal_path)}", fontsize=9, ha='left', va='top')
    ...
    metrics_text = (
        ...
        f"Train Loss: {train_loss_start:.2f} → {train_loss_end:.2f}\n"
        f"Val Loss: {val_loss_start:.2f} → {val_loss_end:.2f}\n"
        f"SUMO start and end edges: {start_edge} → {destination_edge}"
    )
    plt.figtext(0.01, 0.7, metrics_text, fontsize=10, ha='left', va='top')
    
    # Add optimal path if available
    if optimal_path:
        # Format the path - if it's long, we might want to truncate it
        if len(optimal_path) > 10:
            path_str = f"{' → '.join(optimal_path[:3])} ... {' → '.join(optimal_path[-3:])}"
            # Also add the total number of edges
            path_str += f" (total: {len(optimal_path)} edges)"
        else:
            path_str = f"{' → '.join(optimal_path)}"
        plt.figtext(0.01, 0.3, f"Optimal path: {path_str}", fontsize=9, ha='left', va='top')
...
def train_cnn_model(X_train, Y_train, X_test, Y_test, min_speed=None, max_speed=None):
    ...
    optimal_path, min_speed, max_speed = find_optimal_path_all(model, start_edge, destination_edge)
    ...
    f"Min Speed: {min_speed if min_speed is not None else 'N/A'} → Max Speed: {max_speed if max_speed is not None else 'N/A'}"
    ...
    fig = plt.gcf()
    fig.text(0.78, 0.5, training_metrics_summary, transform=fig.transFigure, fontsize=10,
...
# Option 1: Modify find_optimal_path_all to return speed info along with the path
def find_optimal_path_all(model, start_edge, destination_edge):
    try:
        ...
        # Get speed info before closing connection
        ...
        return optimal_path, min_speed, max_speed
    except Exception as e:
...
def count_traffic_lights_in_path(path):
    ...
    # Parameters:    #     path (list): A list of edge IDs representing the route.
    
    # Returns:    #     int: Total count of traffic lights in the path.
...
if __name__ == "__main__":
    ...
    if X_train_image.size and Y_train.size:
        ...
        # Use the modified function to find the optimal path and get speed info
        optimal_path, min_speed, max_speed = find_optimal_path_all(cnn_model, start_edge, destination_edge)
        ...
        # Display the speed range info returned from find_optimal_path_all
