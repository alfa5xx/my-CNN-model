...
def extract_features(edge_id):
    ...
    try:
        ...
        # 3. Speed factor based on vehicle speeds on the edge
        speed_factor = get_vehicle_speed_factor(edge_id)
        
        # Create uniform grids for each feature
        ...
        return np.stack([vehicle_grid, tls_grid, speed_grid], axis=-1) # original  code
    except Exception as e:
...
if __name__ == "__main__":
    ...
    X_train, X_train_scalar, Y_train = collect_training_data(start_edge, destination_edge, simulation_steps=TRAIN_SIM_STEPS)
