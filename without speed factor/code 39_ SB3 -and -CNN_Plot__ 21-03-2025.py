...
TRAIN_EPOCHS = 40         # Number of epochs for training CNN # default is 150
TRAIN_SIM_STEPS = 30      # Simulation steps for training data collection # default is 200
TEST_SIM_STEPS = 10        # Simulation steps for test data collection   # default is 40
...
#-------------------------- new code for store all paths and use them in the future -------------

# Global dictionary to store generated paths for each (start_edge, destination_edge) pair
stored_paths = {}

def save_generated_paths(start_edge, destination_edge, max_depth=50):
    """
    Generate all possible loop-free paths from start_edge to destination_edge
    and save them in a global dictionary.
    """
    paths = generate_all_paths(start_edge, destination_edge, max_depth)
    key = (start_edge, destination_edge)
    stored_paths[key] = paths
    print(f"Saved {len(paths)} paths for {start_edge} to {destination_edge}")
    return paths

def get_saved_paths(start_edge, destination_edge):
    """
    Retrieve saved paths for the specified start and destination edges.
    Returns an empty list if no paths have been stored.
    """
    key = (start_edge, destination_edge)
    return stored_paths.get(key, [])
...
if __name__ == "__main__":
    ...
    if X_train.size and Y_train.size:
        ...
        # Prompt the user for start and destination edges
        nav_start_edge = input("Enter the starting edge (e.g., E10): ").strip()
        nav_destination_edge = input("Enter the destination edge (e.g., E4): ").strip()
        # Retrieve saved paths if available; otherwise, generate and store them
        if not get_saved_paths(nav_start_edge, nav_destination_edge):
            navigation_paths = save_generated_paths(nav_start_edge, nav_destination_edge, max_depth=50)
        else:
            navigation_paths = get_saved_paths(nav_start_edge, nav_destination_edge)
    
        print(f"Navigation paths from {nav_start_edge} to {nav_destination_edge}:")
