...
def select_route_from_training_routes(training_routes):
    """
    Interactive function to select a route from pre-generated training routes
    
    Args:
        training_routes (List[List[str]]): List of previously generated routes
    
    Returns:
        List[str]: Selected route or None if no selection made
    """
    if not training_routes:
        print("No training routes available.")
        return None
    
    while True:
        print("\n--- Route Selection Menu ---")
        print("Available Options:")
        print("1. View all training routes")
        print("2. Select a specific route")
        print("3. Filter routes by criteria")
        print("4. Exit route selection")
        
        choice = input("Enter your choice (1-4): ")
        
        if choice == '1':
            # Display all routes with basic information
            for i, route in enumerate(training_routes, 1):
                print(f"\nRoute {i}:")
                print("Path:", " -> ".join(route))
                
                # Calculate route metrics
                route_length = sum(edge_lengths.get(edge, 0) for edge in route)
                vehicle_count = sum(traci.edge.getLastStepVehicleNumber(edge) for edge in route)
                traffic_lights = sum(predict_tls_count_sb3(edge) for edge in route)
            ...
            try:
                route_index = int(input("Enter route number to select: ")) - 1
                if 0 <= route_index < len(training_routes):
                    selected_route = training_routes[route_index]
                    
                    # Detailed route analysis
                    print("\nSelected Route Analysis:")
                    print("Full Path:", " -> ".join(selected_route))
                    
                    route_length = sum(edge_lengths.get(edge, 0) for edge in selected_route)
                    vehicle_count = sum(traci.edge.getLastStepVehicleNumber(edge) for edge in selected_route)
                    traffic_lights = sum(predict_tls_count_sb3(edge) for edge in selected_route)
            ...
            # Advanced filtering of routes
            print("\n--- Route Filtering Options ---")
            print("1. Filter by maximum length")
            print("2. Filter by maximum vehicle count")
            print("3. Filter by maximum traffic lights")
            
            filter_choice = input("Enter filter option (1-3): ")
            filtered_routes = []
            
            if filter_choice == '1':
                max_length = float(input("Enter maximum route length (meters): "))
                filtered_routes = [
                    route for route in training_routes 
                    if sum(edge_lengths.get(edge, 0) for edge in route) <= max_length
                ]
            
            elif filter_choice == '2':
                max_vehicles = int(input("Enter maximum total vehicles: "))
                filtered_routes = [
                    route for route in training_routes 
                    if sum(traci.edge.getLastStepVehicleNumber(edge) for edge in route) <= max_vehicles
                ]
            
            elif filter_choice == '3':
                max_tls = int(input("Enter maximum traffic lights: "))
                filtered_routes = [
                    route for route in training_routes 
                    if sum(predict_tls_count_sb3(edge) for edge in route) <= max_tls
                ]
            ...
            if filtered_routes:
                ...
                print("No routes match the filtering criteria.")
        
        elif choice == '4':
            print("Exiting route selection.")
            return None
...
if __name__ == "__main__":
    ...
    if X_train.size and Y_train.size:
        ...
        # Add interactive route selection from training routes
    while True:
        print("\n--- Post-Training Options ---")
        print("1. Select a route from training routes")
        print("2. Exit")
        
        choice = input("Enter your choice (1-2): ")
        
        if choice == '1':
            # Ensure TRAINING_ROUTES is populated during initial training
            if 'TRAINING_ROUTES' in locals() or 'TRAINING_ROUTES' in globals():
                selected_route = select_route_from_training_routes(TRAINING_ROUTES)
                if selected_route:
                    # Optional: Perform additional analysis on selected route
                    print("\nSelected Route Details:")
                    print("Path:", " -> ".join(selected_route))
            else:
