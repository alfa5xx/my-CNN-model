...
class SUMOPathEnv(gym.Env):
    ...
    def _is_valid_edge_transition(self, edge_id):
        ...
        current_edge = self.net.getEdge(self.current_path[-1])
       # Instead of calling getToEdge(), directly use the edge objects
        outgoing_edges = {edge.getID() for edge in current_edge.getOutgoing()}  
        return edge_id in outgoing_edges
