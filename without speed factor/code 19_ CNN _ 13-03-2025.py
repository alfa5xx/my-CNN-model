...
def find_optimal_path(model, start_edge, destination_edge, max_steps=30):
    ...
    for step in range(max_steps):
        ...
        # Try to find the best valid next edge
        next_edge = None
        for idx in sorted_indices:
            candidate_edge = index_to_edge[idx]
            
            # Check if this edge is actually connected to the current edge
            if candidate_edge in connectivity_map[current_edge]:
                # Check if taking this edge would create a loop
                if candidate_edge not in visited:
                    next_edge = candidate_edge
                    break
        
        # If no valid next edge found, we're stuck (dead end or all create loops)
        if next_edge is None:
            print("Could not find a valid next edge. Path may be suboptimal.")
            break
        
        # Add the next edge to our path
        optimal_path.append(next_edge)
        visited.add(next_edge)
        current_edge = next_edge
    
    # Evaluate our found path
    path_score = evaluate_path(optimal_path)
    
    # Find the ground truth best path for comparison
    if all_possible_paths:
        best_reference_path = min(all_possible_paths, key=evaluate_path)
        best_reference_score = evaluate_path(best_reference_path)
        
        print("\n=== Path Evaluation ===")
        print(f"CNN Model Path: {optimal_path}")
        print(f"Path Length: {len(optimal_path)} edges")
        print(f"Path Score: {path_score:.2f} (lower is better)")
        
        print(f"\nBest Reference Path: {best_reference_path}")
        print(f"\nBest Reference Path: best_reference_score")