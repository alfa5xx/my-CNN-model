...
def generate_all_paths(start_edge, destination_edge, max_depth= 20): #10):
...
# Add randomized DFS to encourage diverse path exploration
    def dfs_with_randomization(current_edge, current_path, visited):
        if len(current_path) > max_depth or current_edge in visited:
            return
        ...
        if current_edge == destination_edge:
            ...
            # Get all possible next edges and shuffle them for randomization
            next_edges = list(connectivity_map[current_edge])
            np.random.shuffle(next_edges)
            
            for next_edge in next_edges:
                # For longer paths, occasionally allow revisiting edges to find alternative routes
                if len(current_path) > 5 and next_edge in visited and np.random.random() < 0.1:
                    # Temporarily remove from visited to allow exploration
                    visited.remove(next_edge)
                    dfs_with_randomization(next_edge, current_path, visited)
                    visited.add(next_edge)  # Add back to visited
    ...
    # Run multiple times with different random seeds for diverse paths
    for seed in range(5):
        np.random.seed(seed)
        dfs_with_randomization(start_edge, [], set())
    
    # Keep paths of varied lengths for diversity
    short_paths = [p for p in all_paths if len(p) <= max_depth//2]
    medium_paths = [p for p in all_paths if max_depth//2 < len(p) <= 3*max_depth//4]
    long_paths = [p for p in all_paths if len(p) > 3*max_depth//4]
    
    # Ensure representation from all length categories
    final_paths = []
    final_paths.extend(short_paths[:min(len(short_paths), 50)])
    final_paths.extend(medium_paths[:min(len(medium_paths), 100)])
    final_paths.extend(long_paths[:min(len(long_paths), 50)])
    
    print(f"Generated {len(final_paths)} diverse paths from {start_edge} to {destination_edge}")
    return final_paths
...
def extract_features(edge_id):
    # Replace uniform grids with spatial representations
    grid_size = (64, 64)
    features = np.zeros((grid_size[0], grid_size[1], 5))
    
    # Get connected edges to create network topology
    connected_edges = connectivity_map[edge_id]
    
    # Create density gradient based on distance from current edge
    for y in range(grid_size[0]):
        for x in range(grid_size[1]):
            # Calculate position in the grid relative to center
            rel_x = (x - grid_size[0]//2) / (grid_size[0]//2)
            rel_y = (y - grid_size[1]//2) / (grid_size[1]//2)
            
            # Distance from center
            distance = np.sqrt(rel_x**2 + rel_y**2)
            
            # Channel 1: Vehicle density with distance-based falloff
            vehicles = traci.edge.getLastStepVehicleNumber(edge_id)
            edge_length = edge_lengths[edge_id]
            vehicle_density = vehicles / max(edge_length, 1)
            features[y, x, 0] = vehicle_density * max(0, 1 - distance)
            
            # Channel 2: Traffic signal influence with directional component
            angle = np.arctan2(rel_y, rel_x) + np.pi  # 0 to 2π
            sector = int(angle / (2*np.pi/8))  # Divide into 8 sectors
            
            # Get traffic signals in the corresponding direction if available
            if connected_edges and sector < len(connected_edges):
                conn_edge = connected_edges[sector % len(connected_edges)]
                lanes = [lane.getID() for lane in net.getEdge(conn_edge).getLanes()]
                tls_count = sum(sum(1 for link in traci.lane.getLinks(lane) if link[3]) for lane in lanes)
                features[y, x, 1] = tls_count * max(0, 1 - distance)
            
            # Channel 3: Speed ratio with radial gradient
            current_speed = traci.edge.getLastStepMeanSpeed(edge_id)
            max_speed = net.getEdge(edge_id).speed
            speed_factor = current_speed / max_speed if max_speed > 0 else 0
            features[y, x, 2] = speed_factor * (1 - 0.5*distance)
            
            # Additional channel 4: Traffic jam probability
            halting_vehicles = traci.edge.getLastStepHaltingNumber(edge_id)
            total_vehicles = max(1, traci.edge.getLastStepVehicleNumber(edge_id))
            jam_factor = halting_vehicles / total_vehicles
            
            # Additional channel 5: Wait time at intersection
            wait_time = traci.edge.getWaitingTime(edge_id)
            normalized_wait = min(1.0, wait_time / 60.0)  # Normalize to 0-1 (max 60s)
   
            # Apply these new features with spatial variation
            for y in range(grid_size[0]):
                for x in range(grid_size[1]):
                    distance = np.sqrt((x - grid_size[0]//2)**2 + (y - grid_size[1]//2)**2) / (grid_size[0]//2)
                    features[y, x, 3] = jam_factor * max(0, 1 - distance)
                    features[y, x, 4] = normalized_wait * max(0, 1 - distance)  
...
if __name__ == "__main__":
    ...
    X_train, Y_train = collect_training_data(start_edge, destination_edge, simulation_steps=100)
    X_test, Y_test = collect_test_data(start_edge, destination_edge, simulation_steps=20)
    ...
    if X_train.size and Y_train.size:
        ...
        optimal_path = find_optimal_path(model, start_edge, destination_edge, max_steps=30)
